...
# Configuration
NETWORK_FILE = '../../data/slc_network.xml'
ROUTES_FILE = '../../data/compressed_routes.parquet'
OUTPUT_FILE = '../../data/matched_routes.json'
CENTER_LAT = 40.75
CENTER_LON = -111.90
R_EARTH = 6371000
...
def load_network(xml_file):
    ...
    for link in root.find('links'):
        ...
        # Filter for car links
        if 'car' not in modes:
            continue
        
        if from_n in nodes and to_n in nodes:
            # DistanceMatcher uses 'cost'? Or assumes distance?
            # We can verify but simply adding edge is usually enough if coords are good.
            # But let's store length/cost just in case we switch to non-euclidean.
            map_con.add_edge(from_n, to_n) 
            # Note: InMemMap in recent versions computes distance from coords if not provided?
            # Actually, let's trust coordinates for now to avoid attribute complexity mismatch.
            # But filtering 'car' is CRITICAL.
...
def process_routes():
    ...
    # Buffer for incomplete routes at batch boundaries
            ...
            for rid in routes_to_process:
                try:
                    ...
                    if not matched_edges:
                         # if total_processed < 5: print(f"Route {rid}: No match.")
                         total_processed += 1
                         continue
