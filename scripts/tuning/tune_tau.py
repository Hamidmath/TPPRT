...
def main():
    ...
    # Build transition matrix once (alpha_s=0, alpha_l=0, beta=0.2)
    PARAMS['alpha_s'] = 0.0
    PARAMS['alpha_l'] = 0.0
    PARAMS['beta'] = 0.2
    M_2N = build_two_phase_matrix(graph_data)

    # Pre-cache all 49 E_N vectors
    E_N_cache = []
    ...
    # Grid search
    tau_vals = [1.0, 1.2, 1.5, 1.8, 2.0, 2.5, 3.0, 4.0, 5.0]
    damping_vals = [0.85, 0.87, 0.89, 0.91]

    print(f"Tuning tau x damping on {len(sample_times)} timeframes")
    ...
    for tau in tau_vals:
        for damp in damping_vals:
            ...
            for E_N in E_N_cache:
                ...
                # Top-100 MRE (against original E_N)
            ...
            print(f"{tau:6.1f} | {damp:8.2f} | {avg_top100:12.4f} | "
    ...
    if best_feasible:
        ...
        print("No feasible configuration found (all exceeded 0.198 overall).")
        results.sort(key=lambda r: r['overall_mre'])
        best = results[0]
        print(f"Closest: tau={best['tau']}, damping={best['damping']}")
        print(f"  Top-100 MRE: {best['top100_mre']:.4f}")
        print(f"  Overall MRE: {best['overall_mre']:.4f}")

    # Save results
    out_file = config.RESULTS_DIR / 'tau_tuning_results.txt'
    with open(out_file, 'w') as f:
        ...
        f.write("-" * 65 + "\n")
        for r in results:
            f.write(f"{r['tau']:6.1f} | {r['damping']:8.2f} | "
