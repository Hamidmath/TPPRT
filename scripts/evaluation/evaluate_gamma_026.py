...
def main():
    ...
    # We use the defaults: alpha_s=0.0, alpha_l=0.0, beta=0.2
    PARAMS['alpha_s'] = 0.0
    PARAMS['alpha_l'] = 0.0
    PARAMS['beta'] = 0.2
    ...
    loader = np.load(config.POPULARITY_NPZ_026, allow_pickle=True)
    ...
    print(f"Running Baseline PageRank on optimal smoothed target (Gamma=0.26) for {len(sample_times)} timeframes...")
    ...
    avg_top100 = top100_mre_sum / len(sample_times)

    print("-" * 60)
    print(f"Average Top-100 Network MRE (Gamma 0.26): {avg_top100:.4f}")
    print(f"Average Overall Network MRE (Gamma 0.26): {avg_overall:.4f}")
