...
The severity of the sparsity problem is illustrated in Figure~\ref{fig:sparsity}, which shows the distribution of per-window link coverage across all 2,880 timeframes. The vast majority of windows observe fewer than 3\% of links.

\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{fig11_sparsity_histogram.png}
\caption{\textbf{GPS data sparsity.} Distribution of link coverage (fraction of 99,716 links with $\geq 1$ GPS observation) per 15-minute window across all 2,880 timeframes. The median coverage is approximately 2\%, meaning 97--98\% of the network is unobserved in any given window. This extreme sparsity motivates the graph-diffusion smoothing step.}
\label{fig:sparsity}
\end{figure}

Figure~\ref{fig:smoothing} shows the cross-validation results for $\gamma$ selection. The U-shaped curve confirms that both under-smoothing ($\gamma \to 0$, noisy) and over-smoothing ($\gamma \to 1$, blurred) degrade prediction quality, with the optimal tradeoff at $\gamma = 0.26$.

\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{fig6_smoothing_tradeoff.png}
\caption{\textbf{Smoothing parameter cross-validation.} Split-half CV over 49 random timeframes measuring predictive MSE and downstream PageRank MRE as a function of diffusion strength $\gamma$. Both metrics are minimised at $\gamma = 0.26$ (vertical dashed line), with a broad plateau from 0.22 to 0.30. The optimal $\gamma$ retains 69\% of the original count variance while reducing sparsity from 2\% to 100\% link coverage.}
\label{fig:smoothing}
\end{figure}
