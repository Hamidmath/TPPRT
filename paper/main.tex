...
\caption{Cross-validation of smoothing parameter $\gamma$. The optimal $\gamma = 0.26$ minimises both predictive MSE and downstream PageRank MRE.}
\label{fig:smoothing}
\end{figure}
...
\caption{Distribution of link coverage per 15-minute window. Only 1--3\% of the 99,716 links have GPS observations in any given time slot.}
\label{fig:sparsity}
\end{figure}
...
\caption{Self-loop probability as a function of link traversal time for different $\mu$ values. Higher $\mu$ increases mass retention, with the effect most pronounced on long, slow links.}
\label{fig:selfloop}
\end{figure}
...
\caption{Probability mass retention over power iteration steps for three link types at $\mu = 20$. Highway links retain mass much longer than short connectors.}
\label{fig:retention}
\end{figure}
...
\caption{Top-100 and Overall MRE as a function of $\mu$. Both metrics improve monotonically, with diminishing returns beyond $\mu = 20$.}
\label{fig:mre_mu}
\end{figure}
...
\caption{Top-100 MRE heatmap over $\mu$ and $\beta$ parameter space. The optimal region ($\mu = 20$, $\beta = 0.9$) is clearly delineated.}
\label{fig:heatmap}
\end{figure}
...
\caption{Damping factor optimisation. \textbf{(A)}~Baseline sweep without demand weighting: MRE rises with $d$. \textbf{(B)}~With demand weighting ($\gamma$): MRE drops below 0.05 even at $d = 0.96$. \textbf{(C)}~Direct comparison of configurations.}
\label{fig:damping}
\end{figure}
...
\caption{Demand-weighted model validated across three independent train/test splits: temporal (Sept~1--15 vs 16--30), day-of-week (MWF vs TThSSu), and 5-fold random CV.}
...
\caption{I-15 crash analysis four-panel summary. \textbf{(A)}~Smoothed SB traffic. \textbf{(B)}~Raw GPS counts. \textbf{(C)}~PageRank deviation from baseline. \textbf{(D)}~Surface street diversion.}
...
\caption{Football game day analysis. \textbf{(A)}~Event-link timeseries. \textbf{(B)}~PageRank deviation. \textbf{(C)}~$\mu$-control prediction error. \textbf{(D)}~Optimal $\mu$ over time.}
\label{fig:game}
\end{figure}
...
\caption{Cross-validated Tuesday $\mu$ profile. \textbf{(A)}~Median optimal $\mu$ stays in range 10--20. \textbf{(B)}~Per-fold profiles. \textbf{(C)}~MRE at $\mu{=}20$ vs optimal: nearly identical.}
\label{fig:tuesday}
\end{figure}
...
\caption{Ground truth gap vs model error. \textbf{(A)}~Both follow the same temporal pattern. \textbf{(B)}~Model self-distortion is only 3.9\%---PageRank is nearly transparent. \textbf{(C)}~Complete picture: input gap dominates output gap.}
\label{fig:gt_model}
\end{figure}
