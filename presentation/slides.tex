...
\begin{frame}{Two Components of the Model}
  \centering
  \begin{tikzpicture}
    \node[draw, rounded corners, fill=accent!10, minimum width=11cm,
          minimum height=2.6cm, align=center, font=\large] {
      \textbf{Power Iteration:}\quad
      $\displaystyle\mathbf{v}^{(k+1)}
      = \underbrace{d \cdot M_{2N}^\top \cdot \mathbf{v}^{(k)}}_{\text{Transition Matrix (topology)}}
      + \underbrace{(1-d) \cdot E_{2N}}_{\text{Teleportation Vector (ground truth)}}$
    };
  \end{tikzpicture}
...
\begin{frame}{The Two Phases of Driving}
  ...
  \begin{columns}[c]
    \begin{column}{0.45\textwidth}
      \centering
      \textbf{\large Up Phase}\\[6pt]
      Vehicle is \highlight{actively traversing} link $i$\\[4pt]
      Stays on link proportional to\\
      its physical travel time\\[4pt]
      \good{Self-loop traps mass}
    \end{column}
    \begin{column}{0.08\textwidth}
      \centering
      {\Huge$\xrightarrow{\;\beta\;}$}
    \end{column}
    \begin{column}{0.45\textwidth}
      \centering
      \textbf{\large Down Phase}\\[6pt]
      Vehicle is \highlight{locally dispersing}\\[4pt]
      Moves to adjacent links\\
      in the local neighbourhood\\[4pt]
      Represents turning, parking, etc.
    \end{column}
  \end{columns}

  \bigskip
  \centering
  $\beta = 0.90$: probability of dropping from Up to Down per step
\end{frame}
...
\begin{frame}{Self-Loop Probability}
  After adding the self-loop, the row-stochastic transition for link $i$ becomes:
  \[
  ...
  \centering
  \includegraphics[width=0.70\textwidth]{fig2_mass_retention.png}

  \small Links with high $w_i$ retain most of their probability mass
\end{frame}
...
%─────────────────────────────────────────
\begin{frame}{Before \& After Self-Loops}
  \centering
  \includegraphics[width=0.78\textwidth]{fig9_before_after.png}

  \medskip
  \small Self-loops \good{concentrate mass on major arterials}
  instead of spreading it uniformly
\end{frame}
...
\begin{frame}{Key Takeaways}
  \begin{enumerate}
    \item \highlight{Transition matrix} $M_{2N}$ uses \textbf{only topology}\\
          (road graph $+$ lengths $+$ speeds $\to$ two-phase block structure)
    \bigskip
    \item \highlight{Teleportation vector} $E_{2N}$ uses \textbf{only ground truth}\\
          (GPS counts $\to$ Laplace $\to$ smoothing $\to$ normalise)
    \bigskip
    \item \highlight{Smoothing justified} by cross-validation:\\
          $\gamma^* = 0.26$ minimises MSE (interior minimum, U-shaped curve)
    \bigskip
    \item \highlight{Model achieves} MRE = \good{0.19}\\
          ($37{,}800\times$ improvement from smoothing alone)
  \end{enumerate}
\end{frame}
