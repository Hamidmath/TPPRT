...
verify_smoothing.py
====================
Independent verification of the optimal smoothing parameter γ*.
Uses a DIFFERENT random seed (seed=123) and a FINER grid around the
candidate optimal value γ=0.26 to confirm it is a true minimum.

Outputs: smoothing_justification/verification_results.json
Run from project root:
    python3 smoothing_justification/verify_smoothing.py
"""
import numpy as np
...
def main():
    ...
    print("Loading data...")
    raw = load_pop(os.path.join(data, 'popularity_results.npz'))
    graph = load_json(os.path.join(data, 'city_graph_full.json'))
    ...
    # ─── KEY DIFFERENCE: different seed and more splits ───
    random.seed(123)
    ...
    print(f"\n{'='*60}")
    print(f"VERIFICATION RESULTS (seed=123, {valid_count} splits, {n_g} γ values)")
    print(f"{'='*60}")
    print(f"{'γ':>6} | {'MSE_raw':>14} | {'Corr':>8} | {'MSE_PR':>14} | {'Var%':>7}")
    print(f"{'-'*6}-+-{'-'*14}-+-{'-'*8}-+-{'-'*14}-+-{'-'*7}")
    ...
    print(f"\nOptimal γ (min MSE_raw): {opt_raw}")
    print(f"Optimal γ (max corr):    {opt_corr}")
    print(f"Optimal γ (min MSE_PR):  {opt_pr}")

    out = os.path.join(base, 'verification_results.json')
