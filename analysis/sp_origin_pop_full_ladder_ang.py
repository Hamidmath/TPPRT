"""SP origin -> popularity FULL-CORPUS ladder WITH angle term alpha_ang.
...
Per-edge angle feature (i -> j):
    cos_theta(i, j) = unit_v_i . unit_v_j   (unsigned via cos)
    f_ang = 1 + cos_theta + EPS_ANG  in [EPS_ANG, 2+EPS_ANG],
    EPS_ANG = 0.05 to keep negative alpha_ang well-defined.
Per-edge transition weight:
    w(i, j) = speed[j]^alpha_s * lanes[j]^alpha_l * f_ang(i,j)^alpha_ang

Chain alpha = 0.0508 FIXED.

Mode set via TPPR_LADDER_MODE in: lanes_ang, speed_ang, both_ang.

Output: results/sp_origin_pop_full_ladder_ang_<MODE>.json
"""
import json
import os
import sys
...
OUT  = RESULTS / f"sp_origin_pop_full_ladder_ang_{MODE}.json"
...
def main():
    ...
    print(f"[start] SP origin->pop FULL+ANGLE ladder, "
          f"alpha={ALPHA_FIXED} fixed, eps_ang={EPS_ANG}, mode={MODE}, "
    ...
    summary = dict(
        model="SP origin->popularity FULL + angle",
