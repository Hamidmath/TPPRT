"""Professor's two-phase (new-form) ladder, with the chain dynamic
parameters FIXED at the geometric-fit values:
   beta = 0.102 (= 1 / (1 + E[L_up]))
   rho  = 0.101 (= 1 / (1 + E[L_down]))
Five rows on the 840-pair eval:
...
Tuning rows: L-BFGS-B with 5 parallel starts; alpha_s, alpha_l unbounded.

Output: results/tp_new_prof_ladder.json
"""
import json
import os
import random
...
OUT = RESULTS / "tp_new_prof_ladder.json"
...
def _kernel(mode, x):
    ...
    key = ("tp", round(a_s, 4), round(a_l, 4), round(a_len, 4))
...
def _eval_840(P_cs, inputs):
    ...
    for i in range(len(inputs)):
        v = tp_new_iter(P_cs, inputs[i], BETA_FIXED, RHO_FIXED)
...
def main():
    ...
    print(f"[start] TP-new professor ladder, beta={BETA_FIXED}, rho={RHO_FIXED} fixed  "
          f"(parallel max_workers={max_workers})")

    out = dict(model="TP new-form, (beta,rho) fixed at geometric-fit",
                beta_fixed=BETA_FIXED, rho_fixed=RHO_FIXED, rows={})
