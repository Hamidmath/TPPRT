"""SP origin -> popularity FULL-CORPUS ladder.

Train  = every 5-min slot in [2018-08-31 19:00:00 .. 2018-09-23 18:55:00]
         (the first 23 days,  6,624 slots).
Test   = every 5-min slot in [2018-09-23 19:00:00 .. 2018-09-30 18:55:00]
         (the last 7 days,  2,016 slots).

Per slot t: input E_t = diffused trip-ORIGINS at t (Laplace=0.01 + gamma=0.20),
target F_t = diffused popularity at t. Loss = mean Ov.MRE over train slots.

Trained alpha_s and alpha_l are then applied (no retraining) to the 2,016 test
slots; we save both train and test Ov.MRE.

Chain alpha = 0.0508 FIXED.

Mode set via TPPR_LADDER_MODE in: lanes_only, speed_only, both.

Output: results/sp_origin_pop_full_ladder_<MODE>.json
"""
import json
import os
import sys
...
OUT  = RESULTS / f"sp_origin_pop_full_ladder_{MODE}.json"
...
def load_pop_block(npz_path, N, lid_to_idx, t_start, t_end):
    """Return (times, M_dense float32 (T, N)) for slots in [t_start, t_end]."""
...
def main():
    ...
    print(f"[start] SP origin->pop FULL-corpus ladder, "
          f"alpha={ALPHA_FIXED} fixed, mode={MODE}, "
    ...
    summary = dict(
        model="SP origin->popularity FULL",
