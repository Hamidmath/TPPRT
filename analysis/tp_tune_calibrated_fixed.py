...
Output: results/tp_tune_calibrated_fixed.json
"""
import json
import os
import random
...
OUT = RESULTS / "tp_tune_calibrated_fixed.json"
...
def main():
    t0 = time.time()
    print(f"[start] TP tune with beta={BETA_FIXED} rho={RHO_FIXED} fixed, tune (a_s, a_l), "
          f"OV objective")
    ...
    def loss_full(a_s, a_l):
        ...
        for i in range(len(pairs)):
            v = tp_iter(P_cs, inputs[i], BETA_FIXED, RHO_FIXED)
