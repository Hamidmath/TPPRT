"""Old-form TP tuning: minimize Overall MRE on the 840 (t, t+7d) pairs.

Three variants, all with alpha (teleportation rate) fixed at the SP-equivalent
upper bound 0.15:
  lanes_only   tune (beta, alpha_l)
  speed_only   tune (beta, alpha_s)
  both         tune (beta, alpha_s, alpha_l)

5 L-BFGS-B starts each, eps=2e-3, maxiter=20.
beta in [0.01, 0.15] (matches the new-form bound). alpha_s, alpha_l unbounded.

Output: results/tp_old_form_tune_ov.json
"""
import json
import os
import random
...
OUT = RESULTS / "tp_old_form_tune_ov.json"
...
def main():
    t0 = time.time()
    print(f"[start] OLD-form TP tuning, alpha={ALPHA_FIXED} fixed, OV objective")
    ...
    def mk_loss_lanes_only():
        def loss(x):
            beta, a_l = float(x[0]), float(x[1])
            return loss_full(beta, 0.0, a_l)[0]
        return loss
    def mk_loss_speed_only():
        def loss(x):
            beta, a_s = float(x[0]), float(x[1])
            return loss_full(beta, a_s, 0.0)[0]
        return loss
    def mk_loss_both():
        def loss(x):
            beta, a_s, a_l = float(x[0]), float(x[1]), float(x[2])
            return loss_full(beta, a_s, a_l)[0]
        return loss

    VARIANTS = {
        "lanes_only": dict(
            loss=mk_loss_lanes_only(),
            bounds=[(0.01, 0.15), (None, None)],
            starts=[[0.10, 0.0], [0.15, 1.0], [0.05, -1.0],
                    [0.12, 2.0], [0.08, -2.0]],
        ),
        "speed_only": dict(
        ...
        "both": dict(
            loss=mk_loss_both(),
            bounds=[(0.01, 0.15), (None, None), (None, None)],
            starts=[[0.10, 0.0, 0.0], [0.15, 1.0, 1.0], [0.05, -1.0, -1.0],
                    [0.12, 2.0, 0.0], [0.08, 0.0, 2.0]],
        ),
    }
