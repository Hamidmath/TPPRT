"""Old-form TP tuning with alpha_len: tune (beta, alpha_s, alpha_l,
alpha_len) on the SAME 840 (t, t+7d) pairs as Table 10.  alpha
(teleportation rate) fixed at 0.15, same as tp_old_form_tune_ov.py.
...
OUT = RESULTS / "tp_old_tune_with_len.json"
...
STARTS = [
    # beta, alpha_s, alpha_l, alpha_len
    [0.10,  0.0,  0.0,  0.0],
    [0.15,  1.0,  0.5,  0.5],
    [0.05, -1.0, -1.0,  1.0],
    [0.12,  2.65, 0.17, 0.0],
...
def _worker_loss(x):
    beta, a_s, a_l, a_len = float(x[0]), float(x[1]), float(x[2]), float(x[3])
...
def main():
    ...
    b_opt, as_opt, al_opt, alen_opt = best["x_opt"]
    summary = dict(
        model="old-form TP, classical PageRank style, with alpha_len",
        scope="840 (t, t+7d) pairs, Table 10 protocol, alpha=0.15 fixed",
        alpha_fixed=ALPHA_FIXED,
        bounds=dict(beta=[BETA_LO, BETA_HI], alpha_s=None,
                     alpha_l=None, alpha_len=None),
        ...
        best_params=dict(alpha=ALPHA_FIXED, beta=b_opt, alpha_s=as_opt,
                          alpha_l=al_opt, alpha_len=alen_opt),
    ...
    print(f"  best: beta={b_opt:.4f}  alpha_s={as_opt:+.3f}  "
          f"alpha_l={al_opt:+.3f}  alpha_len={alen_opt:+.3f}")
