"""Top-20 busiest within 1 km, ranked on 19:00-19:55 Sept 1 + Sept 22.

Two steps in this single script:
  Step 1. Build the cluster and save the stadium-network figure.
  Step 2. Run the same default + 4D forecast experiment we ran for
          the other clusters, scored only on this 20-link set.
"""
import json
import re
import sys
...
def main():
    ...
    # Rank
    accum = np.zeros(N, dtype=np.float64); n_bins_rank = 0
    for day in RANK_DAYS:
        for t in bins_window(day):
            E = get_E(t)
            if E is not None:
                accum += E; n_bins_rank += 1
    accum /= max(n_bins_rank, 1)
    print(f"[rank] averaged over {n_bins_rank} bins ({WINDOW_START}-{WINDOW_END} on {RANK_DAYS})")
    ...
    np.save(OUT / "top20_busiest_mask.npy", mask)

    # ---------- Step 1: figure ----------
    print("\n[step 1] drawing figure ...")
    ...
    ax.set_title(f"Top-{TOP_K} busiest links within 1 km of Rice-Eccles\n"
    ...
    side.axis("off")
    lines = [
        f"Cluster: top-{TOP_K} busiest in 1 km",
        "  excludes dead-end links",
        "",
        "Ranking source",
        "  days  : Sept 1, Sept 22 (Sat)",
        f"  bins  : 19:00-19:55 ({n_bins_rank} total)",
    ...
    out_png = OUT / "fig_stadium_corridors_top20_busiest.png"
    fig.savefig(out_png, dpi=160, bbox_inches="tight", facecolor="white")
    plt.close(fig)
    print(f"saved {out_png}")

    # ---------- Step 2: experiment ----------
    print("\n[step 2] forecast experiment on the top-20 cluster ...")
    eps = 1e-6
    def run_pair(prior_day, target_day, beta, rho, P_up, P_down):
        d2d, d2m = [], []
