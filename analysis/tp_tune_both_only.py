"""Focused 'both' resubmit: tune (beta, rho, alpha_s, alpha_l) for the new
two-phase form. The original three-mode script (tp_tune_three_ov.py) hit
the 12h SLURM wall on lanes_only + speed_only and never got to 'both'.
This script runs ONLY 'both', warm-started with the best (a_s, a_l)
points from the completed lanes_only / speed_only runs:

  - lanes_only best: beta=rho=0.150, a_s=0, a_l=+1.875  -> Ov 0.962
  - speed_only best: beta=rho=0.150, a_s=+5.136, a_l=0  -> Ov 0.930

Bounds (professor's spec):
  beta, rho  in [0.01, 0.15]
  alpha_s, alpha_l : unbounded

Same sampling / data as tp_tune_three_ov.py:
  - 840 calendar (t, t+7d) pairs, 120/weekday, seed=7
  - prior = diffused E_b(t),  target = diffused E_b(t+7d)
  - diffused = popularity_results_smoothed_osm_gamma020.npz

5 L-BFGS-B starts, Overall MRE objective.

Output: results/tp_tune_both_only.json
"""
import json
import os
import random
...
OUT = RESULTS / "tp_tune_both_only.json"
...
def main():
    ...
    variants = {
        "both": {
            ...
            "starts": [
                [0.15, 0.15, 5.136, 1.875],
                [0.15, 0.15, 5.136, 0.243],
                [0.15, 0.15, 2.674, 0.169],
                [0.10, 0.10, 3.000, 1.000],
                [0.05, 0.05, 1.000, 1.000],
            ],
