"""Top-10, top-20, top-30 busiest-cluster experiment with rankings
built over THREE clean baseline Saturdays (Sept 1, Sept 8, Sept 22),
all at 19:00-19:55. Sept 15 (event day) and Sept 29 (another Utah
home game) are NOT used to rank links.
...
= 225 combos x 12 event bins, scored on all three masks per chain
output. Reports cluster | size | event d2d | d2m default | d2m tuned
...
def main():
    ...
    for k in TOP_KS:
        ...
        masks[k] = m
        np.save(OUT / f"top{k}_busiest_3day_mask.npy", m)
    ...
    print(f"\n[sweep] {len(GRID_BETA)}*{len(GRID_RHO)}*"
          f"{len(GRID_ALPHA_S)}*{len(GRID_ALPHA_L)} = {n_combos} combos")
    ...
    print(f"  alpha_s in {GRID_ALPHA_S} ; alpha_l in {GRID_ALPHA_L}")
    print("=" * 102)
    ...
    with open(OUT / "topk_3day_constrained_4d_results.json", "w") as f:
        json.dump(dict(
    ...
    print(f"\nsaved {OUT/'topk_3day_constrained_4d_results.json'}")
