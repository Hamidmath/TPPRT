...
OUT = RESULTS / "tp_old_fair_top20_5p_event.json"

EPS = 1e-6
TOL, MAX_ITER = 1e-6, 200
# Event-direction pairs: input = Tue Sep 4 (pre-Fair Tue), target =
# Tue Sep 11 (Fair Tue, noon-opening day). 7-day lag matches the
# Table 10 protocol.
PAIRS = [(f"2018-09-04 16:{m:02d}:00", f"2018-09-11 16:{m:02d}:00")
          for m in range(0, 60, 5)]

# Top 20 diffused-busy links in the 1 km Fairpark area, ranked over
# Tue-Thu of W1+W3+W4. All 7 anomaly-lift links are already in the
# top 20. See
# documents/walkthrough2/results/anomalies/fair/region_top20plus7.json
REGION_LINK_IDS = [
...
def main():
    ...
    print(f"  pairs: {len(inputs)}  (each is 1 slot, Sep 4 16:MM -> Sep 11 16:MM)")
