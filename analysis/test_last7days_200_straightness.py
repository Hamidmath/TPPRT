"""Test all 6 trained-with-straightness (angle) rows on 200 random slots from
the last 7 days, seed=99. Same setup as test_last7days_200slots.py but
restricted to the rows with alpha_ang != 0.

Trained alphas from the completed 840-slot angle ladder:

  chain    mode        a_s     a_l     a_ang     train_mean
  SP       ang_only    --      --      -6.252    1.1895
  SP       speed_ang   +5.579  --      -2.300    1.1817
  SP       both_ang    +4.595  +0.929  -2.447    1.1802
  TP-new   ang_only    --      --      -3.037    1.1841
  TP-new   speed_ang   +5.176  --      -2.046    1.1736
  TP-new   both_ang    +4.372  +0.790  -2.169    1.1722
"""
import json
import random
import sys
...
OUT         = ROOT / "results" / "origin_pop_ladder" / "test_last7days_200_straightness.json"
...
# (chain, mode, alpha_s, alpha_l, alpha_ang, train_mean, train_median)
ROWS = [
    ("sp",     "ang_only",   0.0,    0.0,   -6.252, 1.1895, 1.0180),
    ("sp",     "speed_ang", 5.579,   0.0,   -2.300, 1.1817, 1.0088),
    ("sp",     "both_ang",  4.595,   0.929, -2.447, 1.1802, 1.0080),
    ("tp_new", "ang_only",   0.0,    0.0,   -3.037, 1.1841, 1.0113),
    ("tp_new", "speed_ang", 5.176,   0.0,   -2.046, 1.1736, 1.0009),
    ("tp_new", "both_ang",  4.372,   0.790, -2.169, 1.1722, 1.0004),
]
...
def main():
    ...
    print(f"  test pool {len(pool)}, sampled {len(test_slots)} slots "
          f"({test_slots[0]} .. {test_slots[-1]})")

    slot_set = set(test_slots)
    print("[load] diffused popularity + diffused origins ...")
    ...
    print(f"\n[eval] {len(ROWS)} straightness rows on {N_TEST} slots\n")
    print(f"  {'chain':<7} {'mode':<11} {'a_s':>7} {'a_l':>7} {'a_ang':>7}  "
          f"{'TRAIN mean':>11} {'TRAIN med':>10}  {'TEST mean':>10} {'TEST med':>10}")
    out = []
    for chain, mode, a_s, a_l, a_ang, tr_mean, tr_med in ROWS:
        ...
        te_mean = float(np.mean(ov)); te_med = float(np.median(ov))
        print(f"  {chain:<7} {mode:<11} {a_s:+7.3f} {a_l:+7.3f} {a_ang:+7.3f}  "
              f"{tr_mean:11.4f} {tr_med:10.4f}  {te_mean:10.4f} {te_med:10.4f}")
        out.append(dict(chain=chain, mode=mode,
                         alpha_s=a_s, alpha_l=a_l, alpha_ang=a_ang,
                         train_mre_mean=tr_mean, train_mre_median=tr_med,
                         test_mre_mean=te_mean, test_mre_median=te_med,
                         per_slot=ov))
