"""Single-phase tuning with alpha_len: tune (alpha, alpha_s, alpha_l,
alpha_len) on the SAME 840 (t, t+7d) pairs as Table 10.
...
OUT = RESULTS / "sp_tune_with_len.json"
...
STARTS = [
    # alpha, alpha_s, alpha_l, alpha_len
    [0.10,  0.0,  0.0,  0.0],
    [0.15,  1.0,  0.5,  0.5],
    [0.05, -1.0, -1.0,  1.0],
    [0.12,  2.65, 0.17, 0.0],
...
def _worker_loss(x):
    alpha, a_s, a_l, a_len = float(x[0]), float(x[1]), float(x[2]), float(x[3])
...
def main():
    ...
    a_opt, as_opt, al_opt, alen_opt = best["x_opt"]
    summary = dict(
        model="single-phase with alpha_len",
        scope="840 (t, t+7d) pairs, Table 10 protocol",
        bounds=dict(alpha=[ALPHA_LO, ALPHA_HI], alpha_s=None,
        ...
        best_params=dict(alpha=a_opt, alpha_s=as_opt,
    ...
    print(f"  best: alpha={a_opt:.4f}  alpha_s={as_opt:+.3f}  "
