"""SP origin -> popularity ladder WITH per-edge angle exponent alpha_ang.

Adds the turn-angle feature on top of the existing speed / lane scheme.

Per directed adjacency edge (i -> j) we compute the unsigned turn angle
theta(i, j) in [0, pi] from the MATSim XML: each link's direction is
(to_x - from_x, to_y - from_y); cos(theta) = unit_v_i . unit_v_j.
The per-edge angle feature is

    f_ang(i, j) = 1 + cos(theta(i, j)) + EPS_ANG   in [EPS_ANG, 2 + EPS_ANG]

Straight-ahead = 2+eps, perpendicular = 1+eps, U-turn = eps. Unsigned so
left and right turns get the same weight. EPS_ANG floors the U-turn at a
small positive number so a negative alpha_ang does not blow up.

Per-edge transition weight:

    w(i, j) = speed[j]^alpha_s * lanes[j]^alpha_l * f_ang(i, j)^alpha_ang

Only alpha_s, alpha_l, alpha_ang are tuned. Chain alpha = 0.0508 FIXED.

Mode is set via TPPR_LADDER_MODE in:
    ang_only, speed_ang, both_ang

Output: results/sp_origin_pop_ladder_ang_<MODE>.json
"""
import json
import os
import random
...
OUT = RESULTS / f"sp_origin_pop_ladder_ang_{MODE}.json"
...
def parse_link_dirs(xml_path):
    """Read slc_network.xml. Return {link_id: (ux, uy)} unit direction."""
    nodes = {}
    link_dir = {}
...
def build_per_source_arrays(graph, links, lid_to_idx, link_dir):
    """For each source link i, build aligned arrays:
        succ_idx[i] = np.int64 array of successor column indices,
        cos_th[i]  = np.float64 array of cos(theta) for each successor
    in the order the adjacency lists them. Source links with no entry in
    link_dir or no in-graph successors are stored as empty arrays so the
...
def build_P_weighted(N, succ_idx, cos_th, dest_w, a_ang, eps_ang=EPS_ANG):
    """Build the column-stochastic transition with speed/lane dest
    weights AND per-edge (1 + cos theta + eps)^a_ang factors."""
    rows, cols, data = [], [], []
    for i in range(N):
        idxs = succ_idx[i]
        if idxs.size == 0:
...
def _kernel(mode, x):
    ...
    key = ("sp_ang", round(a_s, 4), round(a_l, 4), round(a_ang, 4))
...
def main():
    ...
    print(f"[start] SP origin->pop+ANGLE ladder, alpha={ALPHA_FIXED} fixed, "
          f"eps_ang={EPS_ANG}, mode={MODE}, BPW={BPW}, max_workers={max_workers}")
    ...
    summary = dict(
        model="SP origin->popularity + angle",
