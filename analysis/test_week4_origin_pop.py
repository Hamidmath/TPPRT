"""Held-out test of the origin -> popularity ladder.

Train set = 840 slots from sample_pairs (seed=7, BPW=120). These all live
in weeks where the t+7d sibling also exists, so they are NOT in week 4.

Test set = 100 random 5-min slots from Sep 22 to Sep 28 (week 4),
sampled with seed=42. Disjoint from the training slots by construction.

For each trained mode of the ladder (SP and TP-new), we apply the
already-trained (alpha_s, alpha_l, alpha_ang) with chain dynamics
FROZEN (SP alpha = 0.0508; TP beta = 0.102, rho = 0.101) and report the
mean Overall MRE over the 100 test slots.

Reads:
  data/origins_results.npz
  data/popularity_results_smoothed_osm_gamma020.npz
  data/city_graph_full.json
  data/slc_network.xml
  results/origin_pop_ladder/{sp,tp_new}_origin_pop_ladder[_ang]_<mode>.json
"""
import json
import random
import sys
...
RESULTS_DIR = ROOT / "results" / "origin_pop_ladder"
...
WEEK4_START = "2018-09-22 00:00:00"
WEEK4_END   = "2018-09-28 23:55:00"
    ...
    for ev, el in ET.iterparse(xml_path, events=("start",)):
        if el.tag == "node":
            ...
            if f in nodes and t in nodes:
                ...
                if norm > 0: link_dir[el.get("id")] = (dx / norm, dy / norm)
            el.clear()
    return link_dir
...
def load_trained_alphas(chain, mode, is_angle=False):
    """Return (a_s, a_l, a_ang). For baselines a_s=a_l=a_ang=0."""
    if is_angle:
        f = RESULTS_DIR / f"{chain}_origin_pop_ladder_ang_{mode}.json"
    else:
        f = RESULTS_DIR / f"{chain}_origin_pop_ladder_{mode}.json"
    if not f.exists():
        return None
    d = json.load(open(f))
    if mode in ("uniform_prior", "diffuse_prior"):
        return (0.0, 0.0, 0.0)
    best = d.get("best", {})
    return (float(best.get("alpha_s", 0.0)),
            float(best.get("alpha_l", 0.0)),
            float(best.get("alpha_ang", 0.0)))
...
def main():
    ...
    # Test slot set: 100 random from week 4 (Sep 22 to Sep 28).
    b = load_popularity_npz(SMOOTH_NPZ)
    all_times = [str(t) for t in b["times"]]
    del b
    week4 = [t for t in all_times if WEEK4_START <= t <= WEEK4_END]
    print(f"  week-4 candidate slots: {len(week4)}")
    rng = random.Random(TEST_SEED)
    test_slots = sorted(rng.sample(week4, N_TEST))
    print(f"  test slots: {len(test_slots)}, "
          f"{test_slots[0]} ... {test_slots[-1]}")

    print("[load] diffused popularity (targets) and diffused origins (inputs) ...")
    targets_d = load_diffused_pop(SMOOTH_NPZ, N, lid_to_idx, test_slots)
    inputs_d  = load_diffused_origins(ORIGINS_NPZ, N, lid_to_idx, test_slots, P_diff)
    inputs  = [inputs_d[t]  for t in test_slots]
    targets = [targets_d[t] for t in test_slots]

    # Modes to test.
    NON_ANG = ["uniform_prior", "diffuse_prior", "lanes_only",
                "speed_only", "both"]
    ANG     = ["ang_only", "speed_ang", "both_ang"]

    rows = []
    for chain in ("sp", "tp_new"):
        for mode in NON_ANG:
            params = load_trained_alphas(chain, mode, is_angle=False)
            if params is None: continue
            a_s, a_l, a_ang = params
            rows.append((chain, mode, False, a_s, a_l, a_ang))
        for mode in ANG:
            params = load_trained_alphas(chain, mode, is_angle=True)
            if params is None: continue
            a_s, a_l, a_ang = params
            rows.append((chain, mode, True, a_s, a_l, a_ang))

    print(f"\n[eval] {len(rows)} (chain, mode) rows on {N_TEST} test slots\n")
    print(f"  {'chain':<6} {'mode':<14} {'a_s':>7} {'a_l':>7} {'a_ang':>7}  "
          f"{'test MRE mean':>13} {'test MRE median':>15}")
    u_uniform = np.full(N, 1.0 / N, dtype=np.float64)
    out = []
    for chain, mode, is_ang, a_s, a_l, a_ang in rows:
        ...
        if mode == "uniform_prior":
            # Single chain run with uniform input; same v* compared to every target.
            if chain == "sp":
                v_star = sp_iter(P, u_uniform, SP_ALPHA)
            else:
                v_star = tp_new_iter(P, u_uniform, TP_BETA, TP_RHO)
            for tgt in targets:
                ov_list.append(float(np.mean(np.abs(tgt - v_star) / (tgt + EPS))))
        else:
            for u, tgt in zip(inputs, targets):
                if chain == "sp":
                    v = sp_iter(P, u, SP_ALPHA)
                else:
                    v = tp_new_iter(P, u, TP_BETA, TP_RHO)
                ov_list.append(float(np.mean(np.abs(tgt - v) / (tgt + EPS))))
        mre_mean = float(np.mean(ov_list))
        mre_med  = float(np.median(ov_list))
        tag = mode + ("*" if is_ang else "")
        print(f"  {chain:<6} {tag:<14} {a_s:+7.3f} {a_l:+7.3f} {a_ang:+7.3f}  "
              f"{mre_mean:13.4f} {mre_med:15.4f}")
        out.append(dict(chain=chain, mode=mode, is_angle=is_ang,
                         alpha_s=a_s, alpha_l=a_l, alpha_ang=a_ang,
                         test_mre_mean=mre_mean, test_mre_median=mre_med,
                         per_slot=ov_list))

    out_path = RESULTS_DIR / "test_week4_results.json"
    out_path.write_text(json.dumps(dict(
        n_test=N_TEST, test_seed=TEST_SEED,
        week_range=[WEEK4_START, WEEK4_END],
