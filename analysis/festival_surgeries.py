...
Closure = the EXPANDED red set of i15_crash_event_links_expanded.pdf
(gen_i15_crash_event_links.py with TPPR_EXPAND=1): seed NB I-15 link 59076
plus its one-hop graph neighbours = {59076,59078,59135,59136,84526}.
...
Output: results/origin_pop_ladder/festival_surgeries.json
"""
...
OUT_JSON    = ROOT / "results" / "origin_pop_ladder" / "festival_surgeries.json"
...
# 9th&9th festival two-box closure (gen_festival_event_links.py) + eval box
B1=(40.749590,40.749940,-111.868348,-111.863884)   # EW links on 900 S
B2=(40.747908,40.752069,-111.865526,-111.865140)   # NS links on 900 E
...
def main():
    ...
    for _,el in ET.iterparse(NETWORK_XML,events=("start",)):
        ...
        if f in nodes and t in nodes:
            ...
            if b1 or b2: seed.add(lid)
            if BOX[0]<=mlat<=BOX[1] and BOX[2]<=mlon<=BOX[3]: box.add(lid)
        el.clear()
    closed=set(seed)   # festival: two-box red set only, no neighbor expansion
    box-=closed
    ...
    print(f"  N={N:,} seed={len(seed)} closed(expanded)={len(closed)} box(no-closed)={len(box_idx)}")
    ...
    slots20=[f"2018-09-15 {h:02d}:{m:02d}:00" for h in range(10,18) for m in range(0,60,5)]
    slots13=[f"2018-09-08 {h:02d}:{m:02d}:00" for h in range(10,18) for m in range(0,60,5)]
    ...
    print(f"[load] {'same-day' if SAMEDAY else 'prev-week'} priors + festival-day targets (Sep 15) ...")
    ...
    out=dict(event="I-15 Draper crash 2018-09-20, expanded closure (5 links), prev-week prior",
